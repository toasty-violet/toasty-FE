 ...
 *
 * payerId 를 넘기면 그 구매자로 결제창이 열려 등록해 둔 결제수단을 쓴다.
 * 아직 payerId 가 없는 계좌 등록 단계에서는 비워 두어 비회원으로 연다.
 */
export async function loadPoint3Widgets(
  payerId?: string,
  ...
  const widgets = point3Payments.widgets({ customerKey: payerId ?? ANONYMOUS });
 ...
 * returnPath 는 결제창이 끝나고 돌아올 앱 안의 경로다.
 */
export function requestPoint3Payment(
  widgets: TossPaymentsWidgets,
  {
    ...
    orderName,
    returnPath,
  }: { sessionId: string; orderName: string; returnPath: string },
) {
